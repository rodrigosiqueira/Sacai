...
Setting :: Setting()
{
	this->calibrationPattern = jvr::NOT_EXISTING;
	this->squareSize = 0;
...
void Setting :: interpret()
{
	...
	else
	{
		...
		else
		{
			...
			{
				this->inputType = jvr::VIDEO_FILE;
			}
		}
		//Open 
		if(this->inputType == jvr::CAMERA)
		{
			this->inputCapture.open(this->cameraId);
		}
		else if(this->inputType == jvr::VIDEO_FILE)
		{
			this->inputCapture.open(this->input);
		}
		else if(this->inputType != jvr::IMAGE_LIST && !this->inputCapture.isOpened())
		{
			this->inputType = jvr::INVALID;
		}
	}
	
	if(this->inputType == jvr::INVALID)
	{
	...
	{
		this->calibrationPattern = jvr::CHESSBOARD;
	}
	else if(!this->patternToUse.compare("CIRCLES_GRID"))
	{
		this->calibrationPattern = jvr::CIRCLES_GRID;
	}
	else if(!this->patternToUse.compare("ASYMMETRIC_CIRCLES_GRID"))
	{
		this->calibrationPattern = jvr::ASYMMETRIC_CIRCLES_GRID;
	}

	if(this->calibrationPattern == jvr::NOT_EXISTING)
	{
...
cv::Mat Setting :: nextImage()
{
	...
	{
		result = cv::imread(this->imageList[this->atImageList++], CV_LOAD_IMAGE_COLOR);
	}
